...
- **Event References**:  
  - [https://schema.ocsf.io/1.4.0/classes/ntp_activity](https://schema.ocsf.io/1.4.0/classes/ntp_activity)  
...
| `connection_info.protocol_name` | "NTP" | String |
...
| `metadata.loggers[].name` | `_system_name` | Name of the system or logging subsystem generating the log entry. | Type is String. |
| `metadata.log_name` | `_path` | Log name. | Type is String. |
| `metadata.uid` | `uid` | Unique ID for the connection. | Type is String. |
...
| OCSF | Raw | Zeek Field Description | Notes |
| :---- | :---- | :---- | :---- |
| `observables[].time` | `ref_time` | Reference timestamp. | Could be mapped to an observable. Time when the system clock was last set or corrected. Type is timestamp_t. |
| `request.time` | `org_time` | Originate timestamp. | Time at the client when the request departed for the server. Type is timestamp_t. |
| `response.time` | `rec_time` | Receive timestamp. | Time at the server when the request arrived from the client. Type is timestamp_t. |
| `end_time` | `xmt_time` | Transmit timestamp. | Time at the server when the response left for the client. Type is timestamp_t. |
| - | `num_exts` | Number of extension fields. | No direct mapping in OCSF 1.4.0 schema. Type is Integer. |
| `auth_info.key_id` | `keyid` | Key identifier. | Used for authenticated NTP. Type is Integer. This could map to authentication information in OCSF 1.4.0. |